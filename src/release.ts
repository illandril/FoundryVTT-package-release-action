...
const release = async (packageReleaseToken: string, manifestUrl: string, dryRun?: boolean) => {
  ...
  if (manifestUrl !== canonicalManifestUrl) {
    warning(`The provided manifest-url did not match the manifest path in the fetched manifest. Using the path defined in the package json: ${canonicalManifestUrl}`);
  }
